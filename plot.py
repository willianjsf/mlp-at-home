...
def carregar_csv(exp_id):
    ...
    if not os.path.exists(nome):
        print(f"  [aviso] arquivo não encontrado: {nome} — experimento {exp_id} ignorado.")
        return None
    df = pd.read_csv(nome)
    return df
...
def grafico_learning_rate():
    ...
    for exp_id, cor in zip(exps, cores):
        ...
        if df is None:
            continue
        ax.plot(df["epoca"], df["erro_medio_mse"],
...
def grafico_acuracias():
    ...
    if 2 in exps_disponiveis:
        ax.axhline(y=ACURACIAS[2], color="#ff7f0e",
                   linestyle="--", linewidth=1, label=f"Baseline ({ACURACIAS[2]:.2f}%)")
        ax.legend()
